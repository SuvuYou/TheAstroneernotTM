...
public class ComputeCubesRock : MonoBehaviour
{
    // WARNING: Needs to be the same as in the MarchingCubesConst.hlsl
    private static readonly int NUM_THREADS = 5;
    ...
    {
        int numberOfThreadsGroupsHorizontally = Mathf.CeilToInt((WorldDataSinglton.Instance.ROCK_SIZE) / NUM_THREADS);
        int numberOfThreadsGroupsVertically = Mathf.CeilToInt((WorldDataSinglton.Instance.ROCK_SIZE) / NUM_THREADS);
        ...
        _computeShader.SetInt("numberOfVerticesHorizontally", WorldDataSinglton.Instance.ROCK_SIZE);
        _computeShader.SetInt("numberOfVerticesVertically", WorldDataSinglton.Instance.ROCK_SIZE);
    ...
    {
        int verticesCount = (int)Mathf.Pow(WorldDataSinglton.Instance.ROCK_SIZE, 4);
